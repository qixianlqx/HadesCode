...
function magnifier(fatherName, MainWidth, MainHeight, img_src, SelectWidth, SelectHeight, BigBoxWidth) {
    ...
    $(".magnifierMainBox").mouseenter(function() {
        ...
        var _left = e.clientX + $("body").scrollLeft() - $(".magnifierMainBox").position().left - SelectWidth / 2;
        var _top = e.clientY + $("body").scrollTop() - $(".magnifierMainBox").position().top - SelectHeight / 2;
        if (_left < 0) {
            _left = 0;
        }
